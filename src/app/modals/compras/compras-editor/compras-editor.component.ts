...
export class ComprasEditorComponent implements OnInit, AfterViewInit {
  ...
  enableBtn(): boolean {
    ...
    switch (this.slides.realIndex) {
      case 0:
        if (this.form.value.nome && this.form.value.data && this.custosControl.valid) {
          return true;
        } else {
          return false;
        }
  ...
  calcular(){
    ...
    if(produtosRef.length){
      produtosRef.forEach((elem, i)=> {
        const vCompra = this.produtosControl.at(i).get('valor_compra') as FormControl;
        const porcentagem = this.produtosControl.at(i).get('porcentagem') as FormControl;
        const vSugerido = this.produtosControl.at(i).get('valor_sugerido') as FormControl;
        const calcPorc = (parseFloat(vCompra.value)* parseFloat(porcentagem.value))/100;
        const custoPorProd = this.getValorCustos() / this.getQtdProduto();
        const result = calcPorc + parseFloat(vCompra.value) + parseFloat(String(custoPorProd));
 ...
 getValorCustos(): number{ 
    ...
    if(results.length){
      result = Number(this.form.value.custo_frete) + Number(results.reduce((prev, curr)=> prev+curr));
    }else{
